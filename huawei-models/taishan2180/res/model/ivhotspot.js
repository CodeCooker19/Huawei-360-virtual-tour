...
if(onHover)wnd.hotspotsonhover=onHover;
 ...
 wnd.addRefTarget(function(a){
    ...
    case "mousedown":{
            wnd.deselectHotSpotOnUp=false;
            var n=a.hitInfo?a.hitInfo.node:null;
            if(n && wnd.isMarkup(n)){
                a.wnd.resetHotSpotDelay();
                if(!wnd.openHotSpotURL(n))
                wnd.activateHotSpot(n);
		a.doDef=false;
            }else {wnd.deselectHotSpotOnUp=true;
                if(!wnd.defSelection)a.doDef=false;
		}
                    }break;
    case "mouseup":{
